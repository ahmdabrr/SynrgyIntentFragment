...
class SerializableFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnNextFragSerial.setOnClickListener{
            if (etSerialFragName.text.toString().isEmpty() && etFragSeriallUmur.text.toString().isEmpty())
            {
                Toast.makeText(
                    context,
                ...
                val dataSerial = PersonSerial ("$nama", umur)
                val intent = Intent (context, SerializableActivity::class.java)
                intent.putExtra("DataSerial", dataSerial)
